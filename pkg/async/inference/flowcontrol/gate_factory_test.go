...
func TestGateFactory_EmptyGateType(t *testing.T) {
	factory := NewGateFactory("")
	gate, err := factory.CreateGate(pipeline.GateConfig{GateType: ""})
...
func TestGateFactory_RedisGateSharesClient(t *testing.T) {
	factory := NewGateFactory("")
	params := map[string]any{"address": "localhost:6379"}
	gate1, err1 := factory.CreateGate(pipeline.GateConfig{GateType: "redis", GateParams: params})
	gate2, err2 := factory.CreateGate(pipeline.GateConfig{GateType: "redis", GateParams: params})
