...
func TestNewGCPPubSubMQFlow_PoolRequiredAndValidation(t *testing.T) {
	...
	parseTopics := func(jsonStr string) []TopicConfig {
		var topics []TopicConfig
		if err := json.Unmarshal([]byte(jsonStr), &topics); err != nil {
			t.Fatalf("Failed to parse topics JSON: %v", err)
		}
		return topics
	}

	// Case 1: worker_pool_id is missing, and pool "default" does not exist
	topics := parseTopics(`[{"subscriber_id":"sub-1","inference_objective":"obj","igw_base_url":"http://gw"}]`)
	...
	topics = parseTopics(`[{"subscriber_id":"sub-1","worker_pool_id":"custom-pool","inference_objective":"obj","igw_base_url":"http://gw"}]`)
	cfg = Config{ProjectID: "test-project", Topics: topics}
	cfg.ApplyDefaults()
	...
	topics = parseTopics(`[{"subscriber_id":"sub-1","worker_pool_id":"non-existent","inference_objective":"obj","igw_base_url":"http://gw"}]`)
	cfg = Config{ProjectID: "test-project", Topics: topics}
	cfg.ApplyDefaults()
	...
	topics = parseTopics(`[{"subscriber_id":"sub-1","worker_pool_id":"test-pool","inference_objective":"obj"}]`)
	cfg = Config{ProjectID: "test-project", Topics: topics}
	cfg.ApplyDefaults()

	if err := cfg.Validate(); err == nil || !strings.Contains(err.Error(), "igw_base_url must be specified") {
	...
	topics = parseTopics(`[{"subscriber_id":"sub-1","worker_pool_id":"test-pool","inference_objective":"obj","igw_base_url":"http://gw"}]`)
	cfg = Config{ProjectID: "test-project", Topics: topics}
	cfg.ApplyDefaults()
