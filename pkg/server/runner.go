	...
	for poolID, mergedChan := range dispatch.Channels {
		...
		for w := 1; w <= workersCount; w++ {
			...
			go func(mergedChan chan pipeline.EmbelishedRequestMessage) {
				defer wg.Done()
				asyncworker.Worker(ctx, drainCtx, flow.Characteristics(), inferenceClient, mergedChan, flow.RetryChannel(), flow.ResultChannel(), opts.Worker.RequestTimeout, transforms)
			}(mergedChan)
		}
	}
